...
#         text_x = qr_x + qr_size + padding
#         text_y = label_height - padding - 0.55 * cm
        
#         for idx, field in enumerate(selected_fields):
#             field_value = row.get(field, "N/A")
#             font_type = "Helvetica-Bold" if field in bold_fields else "Helvetica"
#             c.setFont(font_type, 6)
#             c.drawString(text_x, text_y - idx * 0.4 * cm, f"{field}: {field_value}")
...
#     st.subheader("Serial Number Range")
#     start_serial = st.number_input("Start Serial Number", min_value=1, value=1)
#     end_serial = st.number_input("End Serial Number", min_value=start_serial, value=len(data))
    
#     serial_numbers = list(range(start_serial, end_serial + 1))
#     data["Serial Number"] = serial_numbers[:len(data)]
#     fieldnames = data.columns.tolist()
...
#         min_val = min(unique_values)
#         max_val = max(unique_values)
#         selected_range = st.slider("Select range", min_value=min_val, max_value=max_val, value=(min_val, max_val))
#         filtered_data = data[(data[range_column] >= selected_range[0]) & (data[range_column] <= selected_range[1])]
#     else:
#         selected_range = st.multiselect("Select values", unique_values, default=unique_values)
#         filtered_data = data[data[range_column].isin(selected_range)]
    
#     st.dataframe(filtered_data)
    
#     label_count = st.number_input("How many labels to print?", min_value=1, max_value=len(filtered_data), value=len(filtered_data))
#     if label_count < len(filtered_data):
#         filtered_data = filtered_data.iloc[:label_count]
...
#     if st.button("Generate PDF"):
#         pdf_output = create_pdf_with_qr_from_data(filtered_data, label_width, label_height, selected_fields, bold_fields, qr_field)
#         st.download_button(label="Download QR Code PDF", data=pdf_output, file_name="QR_Labels.pdf", mime="application/pdf")


#uper is orignal code 
...
def get_secrets():
    try:
        return st.secrets["SQL"]
    except KeyError:
        with open(".streamlit/secrets.toml", "r") as f:
            return toml.load(f)["SQL"]
...
def connect_to_sql_server():
    try:
        connection = pyodbc.connect(
            f"Driver={{{secrets['SQL_DRIVER']}}};"
            f"Server={secrets['SQL_SERVER']};"
            f"Database={secrets['SQL_DATABASE']};"
            f"Trusted_Connection={secrets['SQL_TRUSTED_CONNECTION']};"
        )
        st.success("Connected to SQL Server successfully!")
        return connection
    except Exception as e:
